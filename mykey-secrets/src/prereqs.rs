// prereqs.rs — System prerequisite checks run at startup.
//
...
pub fn check_secure_boot() {
    ...
    if !std::path::Path::new("/sys/firmware/efi/efivars").exists() {
        error!("[prereqs] FATAL: System is not UEFI. Secure Boot cannot be verified. mykey-secrets will not start on non-UEFI systems.");
        eprintln!("[mykey-secrets] FATAL: System is not UEFI. Secure Boot cannot be verified.");
        std::process::exit(1);
    }
    ...
    let sb_bytes = match std::fs::read(EFI_VAR) {
        ...
        Err(e) => {
            ...
            eprintln!("[mykey-secrets] FATAL: Cannot read Secure Boot EFI variable: {e}.");
            std::process::exit(1);
        }
    };
    ...
    if sb_bytes.len() < 5 {
        ...
        eprintln!("[mykey-secrets] FATAL: Secure Boot EFI variable is malformed (too short).");
        std::process::exit(1);
    }

    if sb_bytes[4] != 1 {
        error!("[prereqs] FATAL: Secure Boot is disabled. mykey-secrets requires Secure Boot to be enabled to protect boot-time integrity. Enable Secure Boot in your BIOS/UEFI settings and reboot.");
        eprintln!("[mykey-secrets] FATAL: Secure Boot is disabled.");
        std::process::exit(1);
    }
    ...
    if let Ok(sm_bytes) = std::fs::read(SETUP_MODE_VAR) {
        if sm_bytes.len() >= 5 && sm_bytes[4] == 1 {
            error!("[prereqs] FATAL: Secure Boot is in Setup Mode (not enforcing). Enroll your keys and exit Setup Mode before running mykey-secrets.");
            eprintln!("[mykey-secrets] FATAL: Secure Boot is in Setup Mode (not enforcing).");
            std::process::exit(1);
        }
    }
