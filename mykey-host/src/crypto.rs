// crypto.rs — P-256 key generation, ECDSA signing, authenticatorData assembly,
// and COSE key encoding.  No private key bytes are written to disk here;
// that responsibility belongs to tpm.rs.
...
pub fn generate_credential_keypair() -> Keypair {
    ...
    // Credential ID — 16 cryptographically random bytes
    let mut credential_id = vec![0u8; 16];
    rng.fill_bytes(&mut credential_id);

    // Private key bytes — kept in Zeroizing wrapper
...
///
/// The private key bytes are zeroized immediately after use.
...
/// SHA-256 of the RP ID string, as required by the WebAuthn spec.
...
/// registration responses (AT flag).  Pass `None` for authentication responses.
pub fn build_authenticator_data(
    rp_id: &str,
    sign_count: u32,
    attested_data: Option<(&[u8; 16], &[u8], &[u8])>, // (aaguid, cred_id, cose_key)
) -> Vec<u8> {
    ...
    let mut data = Vec::with_capacity(37 + attested_data.map_or(0, |(_, id, k)| 18 + id.len() + k.len()));
    ...
    if let Some((aaguid, cred_id, cose_key)) = attested_data {
        data.extend_from_slice(aaguid);                              // 16 bytes AAGUID
...
pub fn encode_cose_public_key(x: &[u8], y: &[u8]) -> Vec<u8> {
    ...
    // 3: -7  (alg = ES256)
    cbor_uint(&mut out, 3);
    cbor_neg_int(&mut out, 7); // -(7) = -7

    // -1: 1  (crv = P-256)
    cbor_neg_int(&mut out, 1); // -1
    cbor_uint(&mut out, 1);

    // -2: bstr(x)
    cbor_neg_int(&mut out, 2); // -2
    cbor_bytes(&mut out, x);

    // -3: bstr(y)
    cbor_neg_int(&mut out, 3); // -3
    cbor_bytes(&mut out, y);

    out
}

/// Encode a "none" attestation object (CBOR map) wrapping authenticatorData.
///
/// ```text
/// {
///   "fmt":      "none",
...
// ---------------------------------------------------------------------------
// Base64url helpers (thin wrappers around the base64 engine)
// ---------------------------------------------------------------------------
...
fn cbor_uint(out: &mut Vec<u8>, v: usize) {
    cbor_head(out, 0, v); // major type 0
}

/// Encode a negative integer -n (where n is passed as the positive magnitude).
/// CBOR negative integers: major type 1, value = |n| - 1.
fn cbor_neg_int(out: &mut Vec<u8>, n: usize) {
    cbor_head(out, 1, n - 1); // major type 1
}

fn cbor_bytes(out: &mut Vec<u8>, b: &[u8]) {
    cbor_head(out, 2, b.len()); // major type 2
    out.extend_from_slice(b);
}

fn cbor_text(out: &mut Vec<u8>, s: &str) {
    cbor_head(out, 3, s.len()); // major type 3
